...
export function checkExistKey(config: ProviderSettings | undefined) {
	...
	// Special case for human-relay, fake-ai, claude-code, qwen-code, roo and kilocode providers which don't need any configuration.
	if (
		config.apiProvider &&
		["human-relay", "fake-ai", "claude-code", "qwen-code", "roo", "kilocode"].includes(config.apiProvider) // kilocode_change: add kilocode for anonymous access
	) {
		return true
	}
