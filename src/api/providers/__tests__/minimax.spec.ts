...
describe("MiniMaxHandler", () => {
	...
	describe("API Methods", () => {
		...
		it("should coerce numeric tool_use id to string in tool_call_partial (fixes AI_InvalidResponseDataError)", async () => {
			// MiniMax API may return content_block.id as a number; downstream expects string.
			mockCreate.mockResolvedValueOnce({
				[Symbol.asyncIterator]: () => ({
					next: vitest
						.fn()
			...
			expect(typeof (firstChunk.value as { id?: string }).id).toBe("string")
		})

		it("should yield undefined id when tool_use has no id", async () => {
			mockCreate.mockResolvedValueOnce({
				[Symbol.asyncIterator]: () => ({
					next: vitest
						.fn()
			...
			expect((firstChunk.value as { id?: string }).id).toBeUndefined()
		})
	})
