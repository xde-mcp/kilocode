...
export class MoveOrchestrator {
	...
	async executeMoveOperation(
		...
		try {
			...
			let validatedData: { sourceFile: any; symbol: any }
			try {
				validatedData = await PerformanceTracker.measureStep(opId, "validation", async () => {
					return this.validator.validateWithExceptions(operation, options.batchContext)
				})
				...
				return {
					...
					affectedFiles: operation.selector.filePath ? [operation.selector.filePath] : [],
				}
			}
			...
			refactorLogger.error(`[ERROR] Unexpected error during move operation: ${error}`)
			PerformanceTracker.endTracking(opId)
