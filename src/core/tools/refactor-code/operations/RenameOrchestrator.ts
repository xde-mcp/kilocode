	...
	async executeRenameOperation(operation: RenameOperation): Promise<OperationResult> {
		...
		try {
			...
			if (!Node.isRenameable(symbol.node)) {
				refactorLogger.error(`[ERROR] Symbol cannot be renamed: ${operation.selector.name}`)
				return {
					success: false,
					operation,
			...
			if (conflictCheck.hasConflict) {
				refactorLogger.error(`[ERROR] Naming conflict: ${conflictCheck.message}`)
				return {
					success: false,
					operation,
			...
			refactorLogger.error(`[ERROR] Rename operation failed: ${err}`)
			return {
				success: false,
				operation,
