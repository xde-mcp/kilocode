...
export class MoveValidator {
	...
	/**
	 * Validates a move operation by checking all prerequisites (exception-based).
	 *
	 * This method performs a comprehensive validation of the move operation:
	 * 1. Validates basic operation parameters
	 * 2. Verifies the source file exists and can be accessed
	 ...
	 * @returns Object containing validated sourceFile and symbol
	 */
	async validateWithExceptions(
		operation: MoveOperation,
		batchContext?: { movedSymbols: Map<string, string[]> },
	): Promise<{ sourceFile: SourceFile; symbol: ResolvedSymbol }> {
		// Step 1: Validate operation parameters
		const paramValidation = this.validateParameters(operation)
		if (!paramValidation.success) {
			throw new ValidationError(
				paramValidation.error || "Parameter validation failed",
				paramValidation.affectedFiles,
			)
		}
		...
		const sourceFileValidation = await this.validateSourceFile(operation)
		if (!sourceFileValidation.success) {
			throw new FileNotFoundError(operation.selector.filePath, { originalError: sourceFileValidation.error })
		}
		...
		const symbolValidation = this.validateSymbol(operation, sourceFile)
		if (!symbolValidation.success) {
			throw new SymbolNotFoundError(operation.selector.name, operation.selector.filePath, {
				originalError: symbolValidation.error,
			})
		}
		...
		if (!targetValidation.success) {
			...
			if (targetValidation.error?.includes("already exists")) {
				throw new SymbolConflictError(operation.selector.name, operation.targetFilePath, {
					originalError: targetValidation.error,
				})
			} else {
				throw new ValidationError(
					targetValidation.error || "Target validation failed",
					targetValidation.affectedFiles,
				)
			}
		}

		// If all validations pass, return the validated objects
