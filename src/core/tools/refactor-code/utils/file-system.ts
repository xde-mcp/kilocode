	...
	try {
		...
		while (attempts <= retries && !verified) {
			try {
				...
				if (!verified) {
					console.warn(`[WARNING] File verification failed on attempt ${attempts + 1}. Content mismatch.`)
					// Wait a bit longer before retry
					await new Promise((resolve) => setTimeout(resolve, 100 * (attempts + 1)))
				}
			} catch (error) {
				console.warn(`[WARNING] File verification failed on attempt ${attempts + 1}`, error)
				// Wait a bit longer before retry
				await new Promise((resolve) => setTimeout(resolve, 100 * (attempts + 1)))
			}
		...
		if (!verified) {
			console.error(`[ERROR] Failed to verify file write after ${retries + 1} attempts: ${filePath}`)
			return false
		}

		return true
	} catch (error) {
		console.error(`[ERROR] Failed to write and verify file: ${filePath}`, error)
		return false
	}
}
