...
describe("AgentManagerProvider - CLI backend", () => {
	...
	beforeEach(async () => {
		...
		const execSyncMock = vi.fn(() => "kilocode")

		vi.doMock("node:child_process", () => ({ spawn: spawnMock, execSync: execSyncMock }))
	...
	it("spawns kilocode without shell interpolation for prompt arguments", async () => {
		const childProcess = await (provider as any).startAgentSession('echo "$(whoami)"')
		...
		;(childProcess as any)?.kill?.("SIGTERM")
	})
