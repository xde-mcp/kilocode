...
/**
 * Gets the diff between a feature branch and base branch as an async iterable
 * This version streams the output line by line for better memory efficiency
 * @param featureBranch The feature branch name
 * @param baseBranch The base branch name (usually 'main' or 'develop')
 * @param workspacePath Path to the workspace
 * @yields GitDiffFile objects with type and filePath
 * @throws Error if git command fails
 */
export async function* getGitDiff2(
	featureBranch: string,
	baseBranch: string,
	workspacePath: string,
): AsyncGenerator<GitDiffFile, void, unknown> {
	try {
		// Get the merge base (commit where branches diverged)
		const mergeBase = await collectOutput(
			`git merge-base ${baseBranch} ${featureBranch}`,
		...
		for await (const line of execGetLines({
			cmd: `git diff --name-status ${mergeBase}..${featureBranch}`,
			cwd: workspacePath,
			...
			switch (status[0]) {
				case "A":
					yield { type: "added", filePath }
					break
				case "M":
					yield { type: "modified", filePath }
					break
				case "D":
					yield { type: "deleted", filePath }
					break
				case "R": // Renamed - treat as delete + add
					if (parts.length >= 3) {
						yield { type: "deleted", filePath: parts[1] }
						yield { type: "added", filePath: parts[2] }
					}
					break
				case "C": // Copied - treat as add
					if (parts.length >= 3) {
