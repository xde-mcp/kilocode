...
export class RemoveOrchestrator {
  ...
  async execute(operation: RemoveOperation): Promise<Partial<OperationResult>> {
    ...
    try {
      ...
      if (!sourceFile) {
        return this.createErrorResult(operation, `Source file not found: ${operation.selector.filePath}`, affectedFiles)
      }
      ...
      if (!resolvedSymbol) {
        return this.createErrorResult(operation, `Symbol '${operation.selector.name}' not found in ${operation.selector.filePath}`, affectedFiles)
      }
      ...
      const removalResult = this.symbolRemover.removeSymbol(resolvedSymbol.node, sourceFile, operation.selector.name)
      if (!removalResult.success) {
        return this.createErrorResult(operation, removalResult.error || 'Failed to remove symbol', affectedFiles)
      }
      ...
      if (finalVerification) {
        return this.createErrorResult(operation, `Symbol '${operation.selector.name}' still exists after removal`, affectedFiles)
      }
...
describe('RemoveOrchestrator', () => {
  ...
  beforeEach(() => {
    ...
    orchestrator = new RemoveOrchestrator(
      project,
      symbolResolver,
      symbolRemover,
      fileManager,
...
export async function executeRemoveOperation(
  ...
  const orchestrator = new RemoveOrchestrator(
    project,
    symbolResolver,
    symbolRemover,
    fileManager,
...
export class MoveOrchestrator {
  ...
  async execute(operation: MoveOperation): Promise<Partial<OperationResult>> {
    console.log(`[DEBUG] MoveOrchestrator executing: ${operation.selector.name} from ${operation.selector.filePath} to ${operation.targetFilePath}`)
   ...
   * Extract from: Lines 221-350 in original move operation
   */
  private async extractSymbolContent(resolvedSymbol: ResolvedSymbol, sourceFile: SourceFile): Promise<ExtractedSymbol> {
   ...
   */
  private checkNamingConflicts(targetFile: SourceFile, symbolName: string): { hasConflict: boolean; message?: string } {
   ...
   * Extract from: Lines 650-750 in original move operation
   */
  private async removeSymbolFromSource(sourceFile: SourceFile, resolvedSymbol: ResolvedSymbol, symbolName: string): Promise<void> {
...
describe('MoveOrchestrator', () => {
  ...
  describe('execute', () => {
    ...
    it('should handle naming conflicts', async () => {
      ...
      const targetFile = project.createSourceFile('target.ts', `
...
describe('Full Refactor Integration Tests', () => {
  ...
  describe('Remove Operation End-to-End', () => {
    it('should handle complex removal scenarios exactly like original', async () => {
      ...
      const sourceFile = project.createSourceFile('src/utils/helpers.ts', `
        interface UserData {
          id: string
          name: string
        }
  ...
  describe('Move Operation End-to-End', () => {
    it('should move function with dependencies correctly', async () => {
      const sourceFile = project.createSourceFile('src/user/service.ts', `
      ...
      project.createSourceFile('src/types/User.ts', `
      ...
      project.createSourceFile('src/utils/formatting.ts', `
...
export class OperationLogger {
  ...
  private logs: Array<{ timestamp: Date, level: 'DEBUG' | 'INFO' | 'WARN' | 'ERROR', message: string, context?: any }> = []
