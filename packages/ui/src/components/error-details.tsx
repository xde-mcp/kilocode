...
export function ErrorDetails(props: ErrorDetailsProps) {
  ...
  const rows = () => {
    ...
    result.push({ label: i18n.t("error.details.type"), value: error.name })

    switch (error.name) {
      case "APIError": {
        if (error.data.statusCode) {
          result.push({ label: i18n.t("error.details.statusCode"), value: String(error.data.statusCode) })
        }
        result.push({ label: i18n.t("error.details.retryable"), value: error.data.isRetryable ? "Yes" : "No" })
        if (error.data.responseBody) {
          result.push({ label: i18n.t("error.details.responseBody"), value: error.data.responseBody, pre: true })
        }
      ...
      case "ProviderAuthError": {
        result.push({ label: i18n.t("error.details.provider"), value: error.data.providerID })
        break
      }
      case "ContextOverflowError": {
        if (error.data.responseBody) {
      ...
      case "StructuredOutputError": {
        result.push({ label: i18n.t("error.details.retries"), value: String(error.data.retries) })
        break
      }
      case "MessageOutputLengthError": {
